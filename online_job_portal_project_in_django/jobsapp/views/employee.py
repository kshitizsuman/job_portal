from jobsapp.models import Applicant, Job, User
...
class EmployeeDashboardView(ListView):
    ...
    def get_queryset(self):
        applications = list(Applicant.objects.filter(user_id=self.request.user.id).values())
        #print(list(Applicant.objects.filter(user_id=self.request.user.id).values()))
        job_ids = []
